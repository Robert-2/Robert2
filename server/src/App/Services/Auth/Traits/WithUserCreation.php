...
trait WithUserCreation
{
     ...
     *                                                   permettra de de créer automatiquement un bénéficiaire lié à l'utilisateur.
    ...
    private function createUserFromAttributes(string $identifier, array $attributes, array $config): User
    {
        ...
        $getAttribute = function ($name, $firstOnly = true) use ($attributesMap, $attributes) {
        ...
        if ($existingUser !== null) {
            ...
            return tap($existingUser, function (User $existingUser) use ($identifier, $config) {
        ...
        if (!empty($externalGroups) || $externalGroups === 0) {
            ...
            if (!empty($config['beneficiaryGroups'])) {
                $isBeneficiary = count(array_intersect($externalGroups, (array) $config['beneficiaryGroups'])) > 0;
            }

            // - Attention à l'ordre de priorité des groupes, qui doit être ascendant.
            $group = array_reduce(
                $externalGroups,
                function ($prevGroup, $samlGroup) use ($internalGroups, $config) {
        ...
        try {
            return dbTransaction(function () use (
                $pseudo,
                $email,
                $group,
                $config,
